...
import items1 from "../model/users/userSchema.js"; // Import the 'items1' model
...
router.post("/add", async (req, res) => {
    try {
        ...
        const newProduct = new items1(req.body);
...
router.get("/search", async (req, res) => {
    try {
        ...
        const products = await items1.find(query);
...
router.put("/update/:id", async (req, res) => {
    try {
        ...
        const product = await items1.findById(productId);
...
router.delete("/delete/:_id", async (req, res) => {
    try {
        ...
        const deletedProduct = await items1.findOneAndRemove({
            _id: productId,
        });
